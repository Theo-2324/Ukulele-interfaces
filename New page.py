...
class TagWindow(QWidget):
    ...
    def __init__(self):
        ...
        self.fs.start(driver="alsa")  # Change driver based on OS
        ...
        if self.sfid == -1:
            ...
            print(f"SoundFont loaded successfully. SoundFont ID: {self.sfid}")
            self.fs.program_select(0, self.sfid, 0, 0)  # Use first instrument
        ...
        self.tagSizeInput = QSpinBox()                                #Creates the settings options Removing the settings options from here breaks things so keep 
        self.tagSizeInput.setRange(10, 512)                          #it here just adjust the values to minimise them
        ...
        self.Button.clicked.connect(lambda: self.setSettingsVisible(self.settingsVisible)) #Changed from not self to self
        self.Button.clicked.connect(self.play_note)

        self.Next = QPushButton('Next-Page', self)
        self.Next.clicked.connect(lambda: self.setSettingsVisible(self.settingsVisible))
        self.Next.clicked.connect(self.goForward)

        self.form.layout().addRow('Tag Size', self.tagSizeInput)              #Removes the settings options from here to 
        ...
        self.form.layout().addRow('', self.mouseEnabledInput)                #here
        self.form.layout().addRow('Jokes on you, im the settings now', QLabel('')) #add lines here to add more settings
        self.form.layout().addRow('Definetly not pipes',self.Button)
        self.form.layout().addRow('Next page',self.Next)
    ...
    def play_note(self):
        print("Playing note...")
        self.fs.noteon(0, 60, 100)  # MIDI Note 60 = Middle C
        QTimer.singleShot(1000, lambda: self.fs.noteoff(0, 60))  # Stop after 1 second
    ...
    def goForward(self, event):
        ...
        self.New_window = GridWindowPageTwo()  #Open page two
...
# EVERYTHING UNDER THIS LINE IS FOR PAGE TWO

# GridWindowPageTwo Class
